...
internal static class RegistratorEmission
{
  ...
  private static string GenerateCode(MetadataGroup firstGroup, IEnumerable<MetadataGroup> groups)
  {
    ...
    var models = groups.Select(x => x.Model).ToArray();
    return
      $$"""
        // <auto-generated/>
        ...
        {
        {{string.Join("\n", models.Select(GetRegistrationCode))}}
        return services;
        }
        }
        """;
    ...
    string GetRegistrationCode(ModelMetadata model) =>
      $"services.AddScoped<{model.ModelName}>().AddScoped<Writable{model.ModelName}>();";
  }
}
